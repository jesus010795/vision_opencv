...
def extraer_color(img):
    (B,G,R) = cv2.split(img)
    caracteristicas = np.array([np.mean(B), np.mean(G), np.mean(R), np.std(B), np.std(G), np.std(R)])
    return caracteristicas

def prediccion(caracteristicas, modelo, valores):
...
with open("modeloClasificador.pkl", "rb") as f:
    ...
    try:
        for contorno in contornos[0]:
            ...
            if M['m00'] > 6000:
                ...
                cx = int(M['m10'] / M['m00'])
                cy = int(M['m01'] / M['m00'])
                cv2.putText(copia_imagen, "Prueba", (cx,cy), cv2.FONT_HERSHEY_COMPLEX,  0.8, (255,0,0), 2)
                # print(cx,cy)
                sleep(.5)
                try:
                    caracteristicas_tiempo_real = extraer_color(recorte)
                    # caracteristicas_tiempo_real = np.array([caracteristicas_tiempo_real])
                    # # print(caracteristicas_tiempo_real)
                    # predicc = prediccion(caracteristicas_tiempo_real, modelo, valores)
                except:
                     print(NameError)
                # print(predicc)
